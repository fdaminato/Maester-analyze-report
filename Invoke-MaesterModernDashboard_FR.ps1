...
.NOTES
    ...
    V4: -PromptModuleUpdate offers Y/N for Update-Module Maester -Force before
    importing the module or updating tests; N continues without updating the module.
    V3: Optional -UpdateTests runs Update-MaesterTests from the validated test
    folder before authentication, restores the location and refreshes discovery.
    V2: Prompts for missing account/tenant parameters, detects the current user's
    test folder and names report folders/HTML/CSV with tenant-date-time.
    V1: Client HTML omits collection-tool references, local source paths, module
    versions and raw-report links. Internal reports and diagnostics remain unchanged.

.EXAMPLE
    .\Invoke-MaesterModernDashboard.ps1 -IncludePreview -OpenReport

.EXAMPLE
    .\Invoke-MaesterModernDashboard.ps1 `
        -TestsPath (Join-Path $env:USERPROFILE 'maester-tests') `
...
.EXAMPLE
    ...
    .\Invoke-MaesterModernDashboard.ps1 `
        -ExistingRunFolder '.\contoso-20260917-105400' `
        -OpenReport
#>
...
param(
    ...
    [Parameter()]
    [string]$TranslatorKey = $env:AZURE_TRANSLATOR_KEY,

    [Parameter()]
    [string]$TranslatorRegion = $env:AZURE_TRANSLATOR_REGION,

    [Parameter()]
    [string]$TranslatorEndpoint = 'https://api.cognitive.microsofttranslator.com',

    [Parameter()]
    [switch]$SkipFrenchReport,
...
$script:AssessmentOrganization = $null

# ISE and other PowerShell hosts are not command-line executables.
if ([System.IO.Path]::GetFileNameWithoutExtension($script:PowerShellExecutable) -notin @('pwsh', 'powershell')) {
    ...
    if ($null -eq $shellCommand) {
        throw 'A PowerShell command-line executable could not be found.'
    }
    $script:PowerShellExecutable = $shellCommand.Source
}


function Invoke-OptionalMaesterModuleUpdate {
    do {
        $answer = (Read-Host 'Run Update-Module Maester -Force before continuing? (Y/N)').Trim()
        if ($answer -notmatch '^(?i:y|n)$') {
            Write-Warning 'Enter Y to update the Maester module or N to skip and continue.'
        }
    } until ($answer -match '^(?i:y|n)$')

    if ($answer -ieq 'y') {
        Write-Step 'Updating the Maester module'
        Update-Module Maester -Force -ErrorAction Stop | Out-Null
    }
    else {
        Write-Host 'Maester module update skipped. Continuing with the installed version.' -ForegroundColor DarkGray
    }
}

function Get-DefaultMaesterTestsPath {
    param(
        [Parameter()]
        [bool]$RunningOnWindows = ([System.Environment]::OSVersion.Platform -eq [System.PlatformID]::Win32NT),
    ...
    if ($RunningOnWindows) {
        if ([string]::IsNullOrWhiteSpace($UserProfile)) {
            throw 'The current Windows USERPROFILE could not be determined.'
        }
        return Join-Path -Path $UserProfile -ChildPath 'maester-tests'
    }
    # macOS (and other Unix platforms): use the current PowerShell directory.
    return Join-Path -Path $WorkingDirectory -ChildPath 'maester-tests'
}

function Resolve-MaesterTestFolder {
    param(
        [Parameter()]
        [string]$Path
    )
    ...
    if ([string]::IsNullOrWhiteSpace($candidate)) {
        $candidate = Get-DefaultMaesterTestsPath
    }

    while ($true) {
        try {
            if (-not (Test-Path -LiteralPath $candidate -PathType Container)) {
                throw "The test folder does not exist: $candidate"
            }
            $resolved = Resolve-Path -LiteralPath $candidate -ErrorAction Stop
            if ($resolved.Provider.Name -ne 'FileSystem') {
                throw 'The tests must be in a filesystem folder.'
            }
            $files = @(Get-ChildItem -LiteralPath $resolved.Path -Filter '*.Tests.ps1' -File -Recurse -ErrorAction Stop)
            if ($files.Count -eq 0) {
                throw "No *.Tests.ps1 files were found in: $candidate"
            }
            Write-Host "Test folder: $($resolved.Path)" -ForegroundColor DarkGray
            return [pscustomobject]@{ Path = $resolved.Path; TestFiles = $files }
        }
        catch {
            Write-Warning $_.Exception.Message
        }

        $candidate = (Read-Host 'Enter the full path to maester-tests (or Q to cancel)').Trim().Trim('"').Trim("'")
        if ($candidate -ieq 'q') {
            throw (New-Object System.OperationCanceledException -ArgumentList 'Assessment cancelled before running tests.')
        }
    }
}

function Read-AssessmentConnectionSettings {
    param(
        [Parameter()]
        [string]$UPN,
    ...
    $promptUPN = [string]::IsNullOrWhiteSpace($UPN)
    do {
        if ($promptUPN) {
            $UPN = Read-Host 'Enter the administrator UserPrincipalName'
        }
        $UPN = $UPN.Trim()
        $validUPN = $UPN -match '^[^@\s]+@[^@\s]+\.[^@\s]+$'
        if (-not $validUPN) {
            if (-not $promptUPN) { throw 'UserPrincipalName is not a valid UPN.' }
            Write-Warning 'Enter a complete administrator UPN, such as admin@contoso.onmicrosoft.com.'
        }
    } until ($validUPN)

    $promptTenant = [string]::IsNullOrWhiteSpace($Tenant)
    do {
        if ($promptTenant) {
            $Tenant = Read-Host 'Enter the target Microsoft Entra TenantId'
        }
        $Tenant = $Tenant.Trim()
        $parsedTenant = [guid]::Empty
        $validTenant = [guid]::TryParse($Tenant, [ref]$parsedTenant) -and $parsedTenant -ne [guid]::Empty
        if (-not $validTenant) {
            if (-not $promptTenant) { throw 'TenantId is not a valid nonempty GUID.' }
            Write-Warning 'Enter the Microsoft Entra tenant GUID.'
        }
    } until ($validTenant)

    return [pscustomobject]@{ UserPrincipalName = $UPN; TenantId = $parsedTenant.ToString() }
}

function Get-AssessmentReportBaseName {
    param(
        [Parameter()]
        $Organization,
    ...
    $prefix = ''
    $organizationObject = if ($null -ne $Organization) { [pscustomobject]$Organization } else { $null }
    $domains = @(Get-ObjectValue -InputObject $organizationObject -PropertyName @('verifiedDomains') -DefaultValue @())
    foreach ($domain in $domains) {
        $domainObject = [pscustomobject]$domain
        $initial = Get-ObjectValue -InputObject $domainObject -PropertyName @('isInitial') -DefaultValue $false
        $domainName = [string](Get-ObjectValue -InputObject $domainObject -PropertyName @('name') -DefaultValue '')
        if ($initial -eq $true -and $domainName -match '(?i)^([^.]+)\.onmicrosoft\.com$') {
            $prefix = $matches[1]
            break
        }
    }
    # Offline rebuilds may have only the saved account and tenant display name.
    if (-not $prefix -and $UPN -match '(?i)@([^.]+)\.onmicrosoft\.com$') {
        $prefix = $matches[1]
    }
    if (-not $prefix) {
        $prefix = [string](Get-ObjectValue -InputObject $organizationObject -PropertyName @('displayName', 'TenantName') -DefaultValue '')
    }
    if (-not $prefix) { $prefix = $Tenant }
    if (-not $prefix) { $prefix = 'tenant' }

    $prefix = [regex]::Replace($prefix.ToLowerInvariant(), '[^a-z0-9-]+', '-').Trim('-')
    if (-not $prefix) { $prefix = 'tenant' }
    if ($prefix.Length -gt 80) { $prefix = $prefix.Substring(0, 80).TrimEnd('-') }
    return '{0}-{1}' -f $prefix, $StartedAt.ToString('yyyyMMdd-HHmmss', [System.Globalization.CultureInfo]::InvariantCulture)
}
    ...
    $connectionState.Graph = Invoke-ServiceConnection -Name 'Microsoft Graph' -Required -ScriptBlock {
        ...
        for ($attempt = 1; $attempt -le 3; $attempt++) {
            ...
            try {
                ...
                $script:AssessmentOrganization = [pscustomobject]$organization
...
function ConvertTo-ClientAssessmentText {
    param(
        [Parameter()]
        [AllowEmptyString()]
        [string]$Text
    ...
    # Diagnostic stack traces can include the operator's profile and module path.
    $textWithoutLocalPaths = [regex]::Replace(
        $Text,
        '(?im)(?:/Users/[^\r\n]*|[A-Z]:\\Users\\[^\r\n]*)',
        '[internal diagnostic path]'
    )

    # Keep explanatory link labels, but omit links that identify the collection tool.
    $textWithoutToolLinks = [regex]::Replace(
        $textWithoutLocalPaths,
        '(?i)\[([^\]]*)\]\([^)]*maester[^)]*\)',
        '$1'
    )
    $textWithoutToolLinks = [regex]::Replace(
        $textWithoutToolLinks,
        '(?i)https?://[^\s<>"\)\]]*maester[^\s<>"\)\]]*',
        ''
    )
...
function ConvertTo-ClientAssessmentTests {
    param(
        [Parameter(Mandatory)]
        [object[]]$Tests
    )
    ...
    foreach ($test in $Tests) {
        # Copy the normalized results; internal evidence remains unchanged.
        $clientTest = [ordered]@{}
        foreach ($property in $test.PSObject.Properties) {
            ...
            switch ($property.Name) {
                'Source' { $value = '' }
                'HelpUrl' {
                    if ([string]$value -match '(?i)maester') {
                        $value = ''
                    }
                }
                'Tags' {
                    $value = (@(([string]$value -split ';') | ForEach-Object {
                        $_.Trim()
                    } | Where-Object {
                        $_ -and $_ -notmatch '(?i)maester'
                    }) -join '; ')
                }
            }
            if ($value -is [string]) {
                $value = ConvertTo-ClientAssessmentText -Text $value
            }
            $clientTest[$property.Name] = $value
        }
        [pscustomobject]$clientTest
    }
}
...
function New-ModernDashboardHtml {
    ...
    $dashboardData = [ordered]@{
        tenant = [ordered]@{
            name = ConvertTo-ClientAssessmentText -Text $tenantName
            id = ConvertTo-ClientAssessmentText -Text $tenantId
            generatedAt = $generatedAt.ToString('yyyy-MM-dd HH:mm:ss')
            runDuration = $runDuration
            reportName = [System.IO.Path]::GetFileNameWithoutExtension($OutputPath)
        }
        summary = [ordered]@{
            total = $total
            passed = $passed
        ...
        tests = @(ConvertTo-ClientAssessmentTests -Tests $Tests)
    }
...
<main class="layout">
  ...
  <section class="grid" aria-label="Assessment summary">
    ...
    <article class="card">
      ...
      <div class="card-note" id="durationNote">Complete security assessment</div>
    </article>
...
<footer>
  Security assessment findings and configuration review.
...
function createLinkToken(label, url, tokens) {
  ...
  const token =
    `@@ASSESSMENT_LINK_${tokens.length}@@`;
    ...
    <div class="metadata-grid">
      ${
        items.map(([label, value]) => {
          ...
          const rendered =
            safe
              ? `<a href="${escapeHtml(safe)}" target="_blank" rel="noopener noreferrer">Open documentation ↗</a>`
              : escapeHtml(value);
...
'@
    ...
    if ($html -match '(?i)maester') {
        throw 'Client dashboard contains an internal tool reference and was not written.'
    }
...
if (-not $InternalCleanProcess) {
    ...
    foreach ($entry in $PSBoundParameters.GetEnumerator()) {
        ...
        # Keep Translator credentials out of the child process command line.
        # The clean child process inherits these environment variables.
        if ($entry.Key -eq 'TranslatorKey') {
            $env:AZURE_TRANSLATOR_KEY = [string]$entry.Value
            continue
        }
        if ($entry.Key -eq 'TranslatorRegion') {
            $env:AZURE_TRANSLATOR_REGION = [string]$entry.Value
            continue
        }
        ...
        if ($null -ne $entry.Value) {
            $childArguments += $parameterName
            if ($entry.Key -in @('TestsPath', 'OutputRoot', 'ExistingRunFolder') -and
                -not [string]::IsNullOrWhiteSpace([string]$entry.Value)) {
                $childArguments += $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath([string]$entry.Value)
            }
            else {
    ...
    # Preserve the caller's logical PowerShell location across the clean restart.
    if (-not $PSBoundParameters.ContainsKey('TestsPath') -and
        [string]::IsNullOrWhiteSpace($ExistingRunFolder)) {
        $childArguments += '-TestsPath'
        $childArguments += Get-DefaultMaesterTestsPath
    }
...
$reportBaseName = 'Security-Assessment'
...
if ($useExistingRun) {
    ...
    Then install the tests with:
        ...
        New-Item -ItemType Directory -Path (Join-Path (Get-Location).Path 'maester-tests') -Force
        Set-Location (Join-Path (Get-Location).Path 'maester-tests')
        Install-MaesterTests
"@
    }

    $testSelection = Resolve-MaesterTestFolder -Path $TestsPath
    $resolvedTestsPath = $testSelection.Path

    # Update before importing Maester so the latest installed version is loaded.
    if ($PromptModuleUpdate) {
        Invoke-OptionalMaesterModuleUpdate
        $maesterModule = Get-LatestAvailableModule -Name 'Maester'
        if ($null -eq $maesterModule) {
            throw 'The Maester module could not be found after the update choice.'
        }
    }
    ...
    if ($UpdateTests) {
        Import-LatestModule -Name 'Maester' -Required | Out-Null
        Write-Step 'Updating Maester tests inside the selected test folder'
        Push-Location -LiteralPath $resolvedTestsPath
        try {
        ...
        # The update may add/remove files; discover the tests again before running.
        $testSelection = Resolve-MaesterTestFolder -Path $resolvedTestsPath
    }

    $testFiles = @($testSelection.TestFiles)
    $resolvedOutputRoot = $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($OutputRoot)

    if (-not $SkipConnect) {
        $settings = Read-AssessmentConnectionSettings -UPN $UserPrincipalName -Tenant $TenantId
        ...
        $organizationResponse = Invoke-MgGraphRequest -Method GET `
            -Uri 'https://graph.microsoft.com/v1.0/organization?$select=id,displayName,verifiedDomains' `
            -ErrorAction Stop
        $script:AssessmentOrganization = @($organizationResponse.value) | Select-Object -First 1
        if ($null -eq $script:AssessmentOrganization -or
            [string]$script:AssessmentOrganization.id -ne [string]$graphContext.TenantId) {
            throw 'The existing Graph session did not return its authenticated tenant organization.'
        }
        Write-Warning 'Skipping service connections because -SkipConnect was specified.' 
    }

    $reportBaseName = Get-AssessmentReportBaseName `
        -Organization $script:AssessmentOrganization `
        -UPN $UserPrincipalName -Tenant $TenantId -StartedAt (Get-Date)
    $runFolder = Join-Path -Path $resolvedOutputRoot -ChildPath $reportBaseName
    if (Test-Path -LiteralPath $runFolder) {
        throw "A report folder already exists for this second: $reportBaseName. Run again to create a new report."
    }
    New-Item -Path $resolvedOutputRoot -ItemType Directory -Force | Out-Null
    New-Item -Path $runFolder -ItemType Directory -ErrorAction Stop | Out-Null
...
function Test-MaesterStringNeedsTranslation {
    param(
        [Parameter(Mandatory)]
        [AllowEmptyString()]
        [string]$Text,
    ...
    $technicalProperties = @(
        'Result','Status','Outcome','Severity','Risk','Level','Service','Workload','Product',
        'Tag','Tags','Path','File','Source','ScriptBlockFile','HelpUrl','DocumentationUrl','DocsUrl',
        'Id','ID','ControlId','TenantId','TenantName','Account','UserPrincipalName','UPN',
        'Duration','ExecutionTime','Elapsed','Time','Version','ModuleVersion','Timestamp','Date'
    )
    if ($PropertyName -in $technicalProperties) { return $false }

    if ($Text -match '^https?://') { return $false }
    if ($Text -match '^[A-Fa-f0-9]{8}-(?:[A-Fa-f0-9]{4}-){3}[A-Fa-f0-9]{12}$') { return $false }
...
function Get-MaesterTranslatableStrings {
    param(
        [Parameter(Mandatory)]
        $InputObject,
    ...
    if ($InputObject -is [string]) {
        if (Test-MaesterStringNeedsTranslation -Text $InputObject -PropertyName $PropertyName) {
    ...
    if ($InputObject -is [System.Collections.IDictionary]) {
        foreach ($key in $InputObject.Keys) {
            Get-MaesterTranslatableStrings -InputObject $InputObject[$key] -Collector $Collector -PropertyName ([string]$key)
        }
        return
    }
    ...
    if ($InputObject -is [System.Collections.IEnumerable] -and -not ($InputObject -is [string])) {
        foreach ($item in $InputObject) {
            Get-MaesterTranslatableStrings -InputObject $item -Collector $Collector -PropertyName $PropertyName
        }
        return
    }

    foreach ($property in $InputObject.PSObject.Properties) {
        Get-MaesterTranslatableStrings -InputObject $property.Value -Collector $Collector -PropertyName $property.Name
    }
}

function Get-TranslatorCache {
    ...
    if (Test-Path -LiteralPath $Path -PathType Leaf) {
        try {
            ...
            Write-Warning "Translation cache could not be read and will be rebuilt: $($_.Exception.Message)"
        }
    }
    return $cache
}

function Save-TranslatorCache {
    param(
...
function Invoke-AzureTranslatorBatch {
    param(
        [Parameter(Mandatory)][string[]]$Texts,
        [Parameter(Mandatory)][string]$Key,
        [Parameter()][string]$Region,
        [Parameter(Mandatory)][string]$Endpoint,
        [Parameter()][string]$ToLanguage = 'fr'
    )

    if ($Texts.Count -eq 0) { return @() }

    $uri = $Endpoint.TrimEnd('/') + '/translate?api-version=3.0&to=' + [uri]::EscapeDataString($ToLanguage)
    $headers = @{
    ...
    $body = @($Texts | ForEach-Object { @{ Text = $_ } }) | ConvertTo-Json -Depth 4 -Compress
    $response = Invoke-RestMethod -Method Post -Uri $uri -Headers $headers -Body $body -ContentType 'application/json; charset=UTF-8'

    $translated = [System.Collections.Generic.List[string]]::new()
    foreach ($item in @($response)) {
        if ($null -eq $item.translations -or @($item.translations).Count -eq 0) {
            $translated.Add('')
        }
        else {
            $translated.Add([string]$item.translations[0].text)
        }
    }
    return @($translated)
}

function Get-FrenchTranslationMap {
    param(
        [Parameter(Mandatory)]$MaesterResults,
        [Parameter(Mandatory)][string]$Key,
        [Parameter()][string]$Region,
        [Parameter(Mandatory)][string]$Endpoint,
    ...
    $collector = [System.Collections.Generic.HashSet[string]]::new([System.StringComparer]::Ordinal)
    Get-MaesterTranslatableStrings -InputObject $MaesterResults -Collector $collector

    $cache = Get-TranslatorCache -Path $CachePath
    $pending = [System.Collections.Generic.List[string]]::new()
    foreach ($text in $collector) {
    ...
    if ($pending.Count -gt 0) {
        Write-Step "Translating $($pending.Count) unique Maester text values to French"
    }

    $batch = [System.Collections.Generic.List[string]]::new()
    $batchChars = 0

    function Submit-TranslationBatch {
        param([System.Collections.Generic.List[string]]$Items)
        if ($Items.Count -eq 0) { return }
        $source = @($Items)
        $translated = @(Invoke-AzureTranslatorBatch -Texts $source -Key $Key -Region $Region -Endpoint $Endpoint -ToLanguage 'fr')
        if ($translated.Count -ne $source.Count) {
            throw "Translator returned $($translated.Count) results for $($source.Count) source strings."
        }
        for ($i = 0; $i -lt $source.Count; $i++) {
            $value = $translated[$i]
            if ([string]::IsNullOrWhiteSpace($value)) { $value = $source[$i] }
            $cache[$source[$i]] = $value
        }
        Save-TranslatorCache -Cache $cache -Path $CachePath
    }

    foreach ($text in $pending) {
        $textLength = $text.Length
        if ($batch.Count -ge 90 -or ($batchChars + $textLength) -gt 40000) {
            Submit-TranslationBatch -Items $batch
            $batch = [System.Collections.Generic.List[string]]::new()
            $batchChars = 0
        }
        $batch.Add($text)
...
function ConvertTo-FrenchMaesterObject {
    param(
        [Parameter(Mandatory)]$InputObject,
        [Parameter(Mandatory)][hashtable]$TranslationMap,
    ...
    if ($InputObject -is [string]) {
        if ((Test-MaesterStringNeedsTranslation -Text $InputObject -PropertyName $PropertyName) -and $TranslationMap.ContainsKey($InputObject)) {
            return [string]$TranslationMap[$InputObject]
        }
        return $InputObject
    }
    ...
    if ($InputObject -is [System.Collections.IDictionary]) {
        $result = [ordered]@{}
        foreach ($key in $InputObject.Keys) {
            $result[$key] = ConvertTo-FrenchMaesterObject -InputObject $InputObject[$key] -TranslationMap $TranslationMap -PropertyName ([string]$key)
        }
    ...
    if ($InputObject -is [System.Collections.IEnumerable] -and -not ($InputObject -is [string])) {
        ...
        foreach ($item in $InputObject) {
            $items.Add((ConvertTo-FrenchMaesterObject -InputObject $item -TranslationMap $TranslationMap -PropertyName $PropertyName))
        }
        return @($items)
    }
    ...
    $props = [ordered]@{}
    foreach ($property in $InputObject.PSObject.Properties) {
        $props[$property.Name] = ConvertTo-FrenchMaesterObject -InputObject $property.Value -TranslationMap $TranslationMap -PropertyName $property.Name
    }
    return [pscustomobject]$props
}

function Add-FrenchDashboardUiLocalization {
    param([Parameter(Mandatory)][string]$Path)

    $html = Get-Content -LiteralPath $Path -Raw -Encoding utf8
    $script = @'
<script id="iti365-fr-ui-localization">
(function () {
  const exact = new Map([
    ["Assessment overview", "Vue d’ensemble de l’évaluation"],
    ["Security score", "Score de sécurité"],
    ["Passed", "Réussis"],
    ["Failed", "Échecs"],
    ["Investigate", "À examiner"],
    ["Skipped", "Ignorés"],
    ["Not run", "Non exécutés"],
    ["Errors", "Erreurs"],
    ["Total tests", "Nombre total de tests"],
    ["Result distribution", "Répartition des résultats"],
    ["Severity distribution", "Répartition par gravité"],
    ["Tests by service", "Tests par service"],
    ["Detailed test analysis", "Analyse détaillée des tests"],
    ["All", "Tous"],
    ["Status", "Statut"],
    ["Severity", "Gravité"],
    ["Service", "Service"],
  ...
  function localizeTextNode(node) {
    ...
    if (!trimmed || !exact.has(trimmed)) return;
  ...
  function localizeElement(el) {
    for (const attr of ["placeholder", "aria-label", "title"]) {
      ...
      if (value && exact.has(value.trim())) el.setAttribute(attr, exact.get(value.trim()));
    }
  }
  ...
  function run() {
    ...
    document.querySelectorAll("*").forEach(localizeElement);
  }
...
'@
    ...
    if ($html -match '(?i)</body>') {
        $html = [regex]::Replace($html, '(?i)</body>', ($script + "`r`n</body>"), 1)
    }
    else {
        $html += "`r`n$script"
    }
...
$dashboardPath =
    Join-Path `
        -Path $runFolder `
        -ChildPath ($reportBaseName + '.html')

$frenchJsonPath =
    Join-Path `
        -Path $runFolder `
        -ChildPath 'Maester-Raw-FR.json'

$frenchDashboardPath =
    Join-Path `
        -Path $runFolder `
        -ChildPath ($reportBaseName + '-FR.html')

$translationCachePath =
    Join-Path `
        -Path $resolvedOutputRoot `
        -ChildPath 'Maester-TranslationCache-fr.json'
...
try {
    ...
    if (
        ...
        if ((Get-Command ConvertFrom-Json).Parameters.ContainsKey('Depth')) {
            $jsonParameters.Depth = 100
        }
        $reportData = Get-Content -LiteralPath $jsonPath -Raw -Encoding utf8 |
            ConvertFrom-Json @jsonParameters
    }
    ...
    if ($useExistingRun) {
        $existingName = Split-Path -Path $runFolder -Leaf
        if ($existingName -match '^[a-z0-9-]+-[0-9]{8}-[0-9]{6}$') {
            $reportBaseName = $existingName
        }
        else {
            $savedAccount = [string](Get-ObjectValue -InputObject $reportData -PropertyName @('Account') -DefaultValue '')
            $savedTenant = [string](Get-ObjectValue -InputObject $reportData -PropertyName @('TenantId') -DefaultValue '')
            $savedName = [string](Get-ObjectValue -InputObject $reportData -PropertyName @('TenantName') -DefaultValue '')
            $originalTime = (Get-Item -LiteralPath $runFolder).CreationTime
            $parsedTime = [datetime]::MinValue
            if ([datetime]::TryParseExact($existingName, 'yyyy-MM-dd_HHmmss',
                [System.Globalization.CultureInfo]::InvariantCulture,
                [System.Globalization.DateTimeStyles]::None, [ref]$parsedTime)) {
                $originalTime = $parsedTime
            }
            $reportBaseName = Get-AssessmentReportBaseName `
                -Organization ([pscustomobject]@{ displayName = $savedName }) `
                -UPN $savedAccount -Tenant $savedTenant -StartedAt $originalTime
        }
        $dashboardPath = Join-Path -Path $runFolder -ChildPath ($reportBaseName + '.html')
        $frenchDashboardPath = Join-Path -Path $runFolder -ChildPath ($reportBaseName + '-FR.html')
    }
    ...
    $frenchTests = @()
    if (-not $SkipFrenchReport) {
        if ([string]::IsNullOrWhiteSpace($TranslatorKey)) {
            throw 'French report generation requires an Azure AI Translator key. Provide -TranslatorKey or set AZURE_TRANSLATOR_KEY. TranslatorRegion is optional for a global Translator resource and required for regional or multi-service resources. Use -SkipFrenchReport to generate only the English reports.'
        }

        $translationMap = Get-FrenchTranslationMap `
            -MaesterResults $reportData `
            -Key $TranslatorKey `
        ...
        Write-Step 'Creating Maester-Raw-FR.json'
        $frenchReportData = ConvertTo-FrenchMaesterObject `
            -InputObject $reportData `
            -TranslationMap $translationMap

        $frenchReportData |
            ConvertTo-Json -Depth 100 -WarningAction SilentlyContinue |
            Set-Content -LiteralPath $frenchJsonPath -Encoding utf8

        Write-Step 'Normalizing the French Maester JSON report'
        $frenchTests = @(
            New-NormalizedTestCollection `
                -MaesterResults $frenchReportData
        )

        if ($frenchTests.Count -eq 0) {
            throw 'The French JSON report did not contain any test records.'
        }

        Write-Step 'Creating the French HTML dashboard from Maester-Raw-FR.json'
        New-ModernDashboardHtml `
            -MaesterResults $frenchReportData `
            -Tests $frenchTests `
            -OriginalHtmlFileName ([System.IO.Path]::GetFileName($originalHtmlPath)) `
            -OutputPath $frenchDashboardPath

        Add-FrenchDashboardUiLocalization -Path $frenchDashboardPath

        if (-not (Test-Path -LiteralPath $frenchDashboardPath -PathType Leaf)) {
            throw "French dashboard generation completed without creating the expected file: $frenchDashboardPath"
        }
    }

    @(
        "Dashboard generated successfully: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
        "Source JSON: $([System.IO.Path]::GetFileName($jsonPath))"
        "Output HTML: $([System.IO.Path]::GetFileName($dashboardPath))"
        "Normalized tests EN: $($normalizedTests.Count)"
        $(if (-not $SkipFrenchReport) { "French JSON: $([System.IO.Path]::GetFileName($frenchJsonPath))" })
        $(if (-not $SkipFrenchReport) { "French HTML: $([System.IO.Path]::GetFileName($frenchDashboardPath))" })
        $(if (-not $SkipFrenchReport) { "Normalized tests FR: $($frenchTests.Count)" })
    ) |
    Set-Content `
    ...
    # Technical failures and raw report locations are for internal diagnostics only.
    $safeMessage = 'The client dashboard could not be generated. Contact the assessment team for a corrected report.'
...
<div class="wrap">
    <div class="card">
        ...
        <p>
            The assessment completed, but this dashboard is unavailable.
        </p>
...
if (-not $SkipFrenchReport) {
    Write-Host `
        "French dashboard : $frenchDashboardPath" `
        -ForegroundColor Cyan
    Write-Host `
        "French JSON      : $frenchJsonPath"
}

if ($OpenReport) {
    Invoke-Item `
        -Path $(if (-not $SkipFrenchReport) { $frenchDashboardPath } else { $dashboardPath })
}

[pscustomobject]@{
    RunFolder       = $runFolder
    DashboardHtml   = $dashboardPath
    FrenchHtml      = $(if (-not $SkipFrenchReport) { $frenchDashboardPath } else { $null })
    OriginalHtml    = $originalHtmlPath
    JsonReport      = $jsonPath
    FrenchJson      = $(if (-not $SkipFrenchReport) { $frenchJsonPath } else { $null })
